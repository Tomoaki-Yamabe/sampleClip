...
- [x] 7.5 CloudFrontディストリビューションの定義
...
- [x] 7.9 nuScenes大規模データ処理とローカル検証
  ...
  - nuScenes Miniデータセット全体のダウンロード
  - より多くのシーン（50-100シーン）の処理
  - ローカルDocker環境での動作確認
  - _要件: 1.1, 1.2, 1.3, 1.4_

- [x] 7.9.1 nuScenes Miniデータセットのダウンロード


  - nuScenes公式サイトからMiniデータセット（約10GB）をダウンロード
  - データセットの解凍と配置
  - データ構造の確認
  - _要件: 1.1_

- [x] 7.9.2 大規模シーンデータの抽出


  - 50-100シーンの画像とメタデータを抽出
  - 既存のextract_nuscenes.pyスクリプトを拡張
  - シーン選択基準の設定（多様性を確保）
  - _要件: 1.1_

- [x] 7.9.3 大規模データの埋め込み生成
...
- [x] 7.9.4 ローカルDocker環境での統合テスト


  - docker-composeで全システムを起動
  - 大規模データでの検索パフォーマンステスト
  - レスポンス時間の測定
  - メモリ使用量の確認
  - _要件: 2.3, 3.3_
...
- [x] 8. CDK統合デプロイ
  ...
  - CDK BucketDeploymentでデータとモデルを自動アップロード
  - Lambda Dockerイメージのビルドとデプロイ
  - フロントエンドのビルドとデプロイ
  - 全体の統合デプロイ
  - _要件: 4.1, 4.2, 4.3, 4.4, 4.5, 5.1, 5.2_

- [x] 8.1 Lambda Dockerイメージの準備
...
- [x] 8.2 CDKスタックへのBucketDeployment追加


  - BucketDeploymentコンストラクトの追加
  - ベクトルDBとメタデータの自動アップロード
  - シーン画像の自動アップロード
  - デプロイ時の依存関係設定
  - _要件: 4.3_

- [x] 8.3 フロントエンドビルドのCDK統合


  - Next.jsビルドスクリプトの作成
  - 環境変数の自動注入
  - BucketDeploymentでS3へ自動アップロード
  - CloudFront無効化の自動化
  - _要件: 5.1, 5.2_

- [x] 8.4 統合デプロイスクリプトの作成


  - デプロイ前チェックスクリプト
  - ワンコマンドデプロイスクリプト（deploy.sh/deploy.ps1）
  - ロールバックスクリプト
  - デプロイ後の動作確認スクリプト
  - _要件: 4.1_
